...
namespace SudokuKata
{
    class Program
    {
        static void Play()
        {
            int[] solvedState = CreateSolvedBoard();
            PrintToConsole(solvedState, "\nFinal look of the solved board:");

            int[] initialState = CreateInitialBoard(solvedState);
            ...
            while (changeMade)
            {
                ...
                while (stepChangeMade)
                {
                    (changeMade, stepChangeMade, state) =
                        new SingleCandidateRule(rng, maskToOnesCount, singleBitToIndex)
                            .Then(new SinglePlaceForCandidate(rng))
                            .Then(new TwoNumbersInTwoCellsRule(maskToOnesCount, cellGroups))
                            .Then(new NNumbersInCellsRule(maskToOnesCount, cellGroups))
                ...
                {
                    string code =
                        string.Join(string.Empty, state.Select(s => s.ToString()).ToArray())
        ...
        private static int[] CreateInitialBoard(int[] solvedState)
        {
        ...
        private static string ToPrintableString(int[] board) =>
            board.Select((value, index) => (value: value, row: index / 9, col: index % 9))
                .GroupBy(tuple => tuple.row)
                .OrderBy(group => group.Key)
                .Select(group => (row: group.Key, text:
                    group.OrderBy(tuple => tuple.col)
                        .Select(tuple => (label: tuple.value == 0 ? '.' : (char)('0' + tuple.value), row: tuple.row, col: tuple.col))
                        .Select(tuple => tuple.col % 3 == 0 ? $"|{tuple.label}" : $"{tuple.label}")
                        .Join(string.Empty) + "|"))
                .Select(tuple => tuple.row % 3 == 0 ? $"+---+---+---+\n{tuple.text}" : tuple.text)
